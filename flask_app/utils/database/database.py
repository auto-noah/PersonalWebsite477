...
class database:
    ...
    # Pulls data from database to genereate data
    def getResumeData(self):
        ...
        for inst in institution_query:
            ...
            # Get position id for each position and query for applicable experiences from position
            for pos in position_query:
                position_id = pos['position_id']
                pos.pop('position_id')
                pos.pop('inst_id')

                experiences = dict()
                experiences_query = self.query(f'SELECT * FROM experiences WHERE position_id = {position_id}') # Get experience data

                # Get experience id for each experience and query for applicable skills learned during the experience
                for exp in experiences_query:
                    experience_id = exp['experience_id']
                    exp.pop('experience_id')
                    exp.pop('position_id')

                    skills = dict()
                    skills_query = self.query(f'SELECT * FROM skills WHERE experience_id = {experience_id}') # Get skill data

                    # Get skill id for each skill from the experience and start buidling the resumeData dict
                    for skill in skills_query:
                        skill_id = skill['skill_id']
                        skill.pop('skill_id')
                        skill.pop('experience_id')

                        skills[skill_id] = skill

                    # Connect skills to an experience and store it
                    exp['skills'] = skills
                    experiences[experience_id] = exp

                # Connect experiences to a position and store it
                pos['experiences'] = experiences
                positions[position_id] = pos

            # Connect positions to an institution and store it
